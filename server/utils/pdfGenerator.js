...
// Ensure a value is a full URL with protocol
const makeUrl = (val) => {
  if (!val) return '#';
...
const generatePdfUtil = async (resumeData) => {
  ...
  const modernCss = `
    *{box-sizing:border-box;margin:0;padding:0;}
    body{font-family:${fontStack};color:#1f2937;background:white;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
    .page{padding:20mm;min-height:297mm;background:white;}
    header{margin-bottom:2rem;border-bottom:4px solid ${themeColor};padding-bottom:1rem;}
    h1{font-size:3rem;font-weight:900;text-transform:uppercase;letter-spacing:-0.05em;color:#111827;line-height:1;}
    .contact-row{display:flex;flex-wrap:wrap;column-gap:1rem;row-gap:0.25rem;margin-top:1rem;font-size:0.875rem;font-weight:600;color:#6b7280;}
    .contact-row a{color:#6b7280;text-decoration:none;}
    .contact-row a:hover{text-decoration:underline;}
    .section{margin-bottom:1.5rem;}
    .section-title{font-size:1.125rem;font-weight:700;text-transform:uppercase;border-bottom:2px solid ${themeColor};margin-bottom:0.625rem;padding-bottom:2px;letter-spacing:0.025em;color:#111827;}
    ...
    .date-label{font-size:0.75rem;font-weight:600;color:#6b7280;text-transform:uppercase;white-space:nowrap;margin-left:1rem;flex-shrink:0;}
    .desc-italic{font-size:0.875rem;margin-top:0.25rem;color:#374151;line-height:1.625;font-style:italic;}
    ...
    .skills-grid{display:grid;grid-template-columns:1fr 1fr;column-gap:2rem;row-gap:0.5rem;font-size:0.875rem;color:#374151;}
    .link-text{font-size:0.75rem;font-weight:700;color:${themeColor};margin-left:1rem;text-decoration:none;}
    .tags{display:flex;flex-wrap:wrap;gap:0.5rem;margin-top:0.5rem;}
    .tag{font-size:10px;padding:2px 8px;border-radius:4px;font-weight:500;background-color:${themeLight};color:${themeColor};}
            ...
            ${d.projects.map(proj => `
              <div>
                <div class="row-between">
                  ...
                  ${proj.link ? `<a href="${makeUrl(proj.link)}" class="link-text">${proj.link}</a>` : ''}
                </div>
  ...
  const classicCss = `
    *{box-sizing:border-box;margin:0;padding:0;}
    body{font-family:${fontStack};color:#000;background:white;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
    .page{padding:20mm;min-height:297mm;background:white;}
    header{margin-bottom:3rem;text-align:center;border-bottom:1px solid ${themeMid};padding-bottom:2rem;text-transform:uppercase;letter-spacing:0.1em;}
    h1{font-size:3rem;font-weight:400;letter-spacing:0.1em;color:${themeColor};margin-bottom:1rem;}
    .contact-row{display:flex;justify-content:center;flex-wrap:wrap;column-gap:1.5rem;row-gap:0.5rem;font-size:0.75rem;font-style:italic;color:#6b7280;}
    .contact-row a{color:#6b7280;text-decoration:none;}
    .section{margin-bottom:2rem;}
    .section-title{font-size:1.125rem;font-weight:700;text-transform:uppercase;text-align:center;border-bottom:1px solid ${themeMid};margin-bottom:1rem;padding-bottom:0.5rem;letter-spacing:0.1em;color:${themeColor};}
    ...
    .item-title{font-weight:700;font-size:0.875rem;color:#111827;margin-bottom:0.25rem;}
    ...
    switch (id) {
      ...
      case 'skills': return d.skills?.length ? `
        <section class="section">
          ...
          <div class="space-y-sm">
            ${d.skills.map(s => `<p style="font-size:0.875rem;"><strong class="skill-cat">${s.category}:</strong> ${(s.items || []).join(', ')}</p>`).join('')}
          </div>
        </section>` : '';
      ...
      case 'projects': return d.projects?.length ? `
        <section class="section">
          ...
          <div class="space-y">
            ${d.projects.map(proj => `
              <div>
                <div class="item-title">${proj.name}${proj.link ? ` — <a href="${makeUrl(proj.link)}" style="color:${themeColor};font-weight:700;font-size:0.75rem;text-decoration:none;">${proj.link}</a>` : ''}</div>
  ...
  const minimalistCss = `
    *{box-sizing:border-box;margin:0;padding:0;}
    body{font-family:${fontStack};color:#111827;background:white;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
    ...
    .contact-row{display:flex;flex-wrap:wrap;column-gap:2rem;row-gap:0.5rem;font-size:10px;color:#9ca3af;text-transform:uppercase;letter-spacing:0.1em;font-weight:700;}
    .contact-row a{color:#9ca3af;text-decoration:none;}
    .section{margin-bottom:3rem;}
    .section-title{font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:0.4em;color:#9ca3af;margin-bottom:1.5rem;}
    ...
    .proj-link{font-size:0.75rem;color:${themeColor};text-decoration:none;font-weight:600;margin-left:0.5rem;}
    ...
    .skill-item{font-weight:500;margin-bottom:0.25rem;font-size:0.875rem;color:#374151;}
        ...
        <section class="section">
          ...
          ${d.projects.map(proj => `
            <div class="proj-item">
              <div class="proj-name">
                ${proj.name}
                ${proj.link ? `<a href="${makeUrl(proj.link)}" class="proj-link">${proj.link}</a>` : ''}
              </div>
  ...
  const executiveCss = `
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .contact-row{display:flex;flex-wrap:wrap;column-gap:1.5rem;row-gap:0.5rem;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:0.1em;color:#9ca3af;}
    .contact-row a{color:#9ca3af;text-decoration:none;}
    .contact-link{color:${themeColor} !important;}
    .section{margin-bottom:2.5rem;}
    .section-title{font-size:0.875rem;font-weight:900;text-transform:uppercase;letter-spacing:0.1em;border-left:4px solid ${themeColor};padding-left:1rem;margin-bottom:1.5rem;color:${themeColor};}
    ...
    .proj-link{color:${themeColor};font-weight:700;text-decoration:none;font-size:0.7rem;margin-left:0.5rem;}
    ...
    .skill-item{font-weight:600;margin-bottom:0.25rem;font-size:11px;color:#374151;}
  ...
  // ── Select template ─────────────────────────────────────────────
  let css, sectionFn, headerHtml, footerHtml = '';

  const contactHtml = (labelLinkedIn = 'LinkedIn', labelGitHub = 'GitHub', linkClass = '') => `
    ${d.personalInfo?.email ? `<a href="mailto:${d.personalInfo.email}">${d.personalInfo.email}</a>` : ''}
    ${d.personalInfo?.phone ? `<span>${d.personalInfo.phone}</span>` : ''}
    ${d.personalInfo?.location ? `<span>${d.personalInfo.location}</span>` : ''}
    ${d.personalInfo?.linkedin ? `<a href="${makeUrl(d.personalInfo.linkedin)}"${linkClass ? ` class="${linkClass}"` : ''}>${labelLinkedIn}</a>` : ''}
    ${d.personalInfo?.github ? `<a href="${makeUrl(d.personalInfo.github)}"${linkClass ? ` class="${linkClass}"` : ''}>${labelGitHub}</a>` : ''}`;
  ...
  if (template === 'classic') {
    ...
    headerHtml = `
      <header>
        ...
        <div class="contact-row">
          ...
          ${d.personalInfo?.email ? `<a href="mailto:${d.personalInfo.email}">${d.personalInfo.email}</a>` : ''}
          ${d.personalInfo?.phone ? `<span>${d.personalInfo.phone}</span>` : ''}
          ${d.personalInfo?.linkedin ? `<a href="${makeUrl(d.personalInfo.linkedin)}">li</a>` : ''}
          ${d.personalInfo?.github ? `<a href="${makeUrl(d.personalInfo.github)}">gh</a>` : ''}
        </div>
      </header>`;
    ...
    headerHtml = `
      <header>
        <h1>${d.personalInfo?.name || 'Your Name'}</h1>
        <div class="contact-row">${contactHtml('LI', 'GH', 'contact-link')}</div>
      </header>`;
    footerHtml = `<footer>Private &amp; Confidential &mdash; Generated by Resumify Elite</footer>`;
